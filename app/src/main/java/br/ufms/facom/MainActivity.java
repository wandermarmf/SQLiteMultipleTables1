...
import br.ufms.facom.view.ViewToDoAdapter;
...
public class MainActivity extends Activity
{
	...
	AdapterView.OnItemClickListener adapterViewListener = new AdapterView.OnItemClickListener() {
		...
		public void onItemClick(AdapterView<?> parent, View view, int position, long id) {
			...
			if ((position > -1) && (position < listTodos.size())) {
				todo = listTodos.get(position);
			}

			Intent intent = new Intent(MainActivity.this, TodoEditActivity.class);

			TransportTodo.setObjeto(todo);
	...
	protected void onCreate(Bundle savedInstanceState)
	{
		...
		lvListaTodo = (ListView)findViewById(R.id.lvListaTodo);
		db = new DatabaseHelper(getApplicationContext());

		listTodos = db.getAllToDos();

		// adiciona manipulador do click
		lvListaTodo.setOnItemClickListener(adapterViewListener);

		// carrega a lista de todos do banco de dados
		//db.loadTodoToListView(lvListaTodo, getBaseContext());
		ViewToDoAdapter adapter = new ViewToDoAdapter(getBaseContext(), listTodos);
		lvListaTodo.setAdapter(adapter);
...
/*
		...
		long todo1_id = db.createToDo(todo1, new long[]
		{ tag1_id });
		long todo2_id = db.createToDo(todo2, new long[]
		{ tag1_id });
		long todo3_id = db.createToDo(todo3, new long[]
		{ tag1_id });

		// Inserting todos under "Watchlist" Tag
		long todo4_id = db.createToDo(todo4, new long[]
		{ tag3_id });
		long todo5_id = db.createToDo(todo5, new long[]
		{ tag3_id });
		long todo6_id = db.createToDo(todo6, new long[]
		{ tag3_id });
		long todo7_id = db.createToDo(todo7, new long[]
		{ tag3_id });

		// Inserting todos under "Important" Tag
		long todo8_id = db.createToDo(todo8, new long[]
		{ tag2_id });
		long todo9_id = db.createToDo(todo9, new long[]
		{ tag2_id });

		// Inserting todos under "Androidhive" Tag
		long todo10_id = db.createToDo(todo10, new long[]
		{ tag4_id });
		long todo11_id = db.createToDo(todo11, new long[]
		{ tag4_id });
